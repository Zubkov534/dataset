...
# Индивидуальные параметры рисования для студенческого ID 70220601:
# рекурсивная сумма цифр ID = 9; толщина = 9 // 2 + 5 = 9;
# последние 6 цифр ID = 220601 => RGB(22, 6, 1) => #160601.
DEFAULT_DRAW_COLOR = "#160601"
DEFAULT_DRAW_WIDTH = 9
...
class DrawVisualApp:
    def __init__(self, root):
        ...
        self.draw_width = DEFAULT_DRAW_WIDTH
    ...
    def set_x_column(self, column):
        ...
        self.exit_draw_mode("Рисование выключено: изменена ось X")
        self.drawn_lines.clear()
        self.draw_graph()

    def set_y_column(self, column):
        self.y_column = column
        self.exit_draw_mode("Рисование выключено: изменена ось Y")
        self.drawn_lines.clear()
        self.draw_graph()
    ...
    def choose_draw_color(self):
        selected_color = colorchooser.askcolor(color=self.draw_color, title="Выбор цвета линии")
        if selected_color and selected_color[1]:
            self.draw_color = selected_color[1]
            self.color_button.config(background=self.draw_color)

    def change_draw_width(self):
        try:
            width = int(self.width_spinbox.get())
            self.draw_width = max(1, min(10, width))
        except ValueError:
            self.draw_width = DEFAULT_DRAW_WIDTH

    def toggle_draw_mode(self):
        if self.draw_mode:
            self.exit_draw_mode("Рисование выключено")
        else:
            self.draw_mode = True
            self.current_line_points = []
    ...
    def exit_draw_mode(self, message="Рисование выключено"):
        if self.draw_mode:
            self.draw_mode = False
            self.current_line_points = []
            self.draw_button.config(
    ...
    def on_mouse_press(self, event):
        if self.draw_mode and event.button == 3:
            self.exit_draw_mode("Рисование выключено: нажата правая кнопка мыши")
            return
    ...
    def undo_last_line(self, event=None):
        if self.current_line_points:
            self.status_label.config(text="Отмена недоступна до завершения текущей линии")
            return
